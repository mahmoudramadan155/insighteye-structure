...
class StreamManager:
    ...
    async def start_stream_background(
        ...
        async with self._safe_stream_operation(stream_id_str, "start"):
            ...
            try:
                ...
                logger.info(f"📊 Database verification: is_streaming={verify_result['is_streaming']}, "
                        f"status={verify_result['status']}")
                
                if not verify_result['is_streaming'] or verify_result['status'] == 'error':
                    logger.error(
                        f"❌ DATABASE INCONSISTENCY DETECTED! "
                        f"Stream {stream_id_str} should be running but DB shows: "
                        f"is_streaming={verify_result['is_streaming']}, status={verify_result['status']}"
                    )
                    # Force correct it
                    await self.db_manager.execute_query(
                        """UPDATE video_stream 
                        SET is_streaming = TRUE, status = 'processing', updated_at = NOW() 
                        WHERE stream_id = $1""",
                        (stream_id,)
                    )
                    logger.info(f"✅ Corrected database state for {stream_id_str}")
                ...
                # ===== CRITICAL FIX: Ensure Qdrant collection BEFORE starting =====
                ...
                try:
                    ...
                    # Don't fail the stream start, but log the issue
    ...
    async def start_stream_in_workspace(
        ...
        # CRITICAL FIX: Set is_streaming=TRUE in a single atomic operation
        # This prevents the management loop from seeing intermediate states
        await self.db_manager.execute_query(
            """UPDATE video_stream 
        ...
        # The management loop will pick this up and start the stream
    ...
    async def stop_stream_in_workspace(
        ...
        # CRITICAL FIX: Use a flag instead of direct database update
        # This prevents race conditions with the management loop
        stream_id_str = str(stream_id)
        
        # Set stop flag in memory first
        async with self._lock:
            if stream_id_str in self.active_streams:
                ...
                if stop_event:
                    stop_event.set()
                    logger.info(f"🛑 Stop event set for stream {stream_id_str}")
                
                # Mark as stopping (but keep is_streaming=TRUE temporarily)
                self.active_streams[stream_id_str]['status'] = 'stopping'
        
        # Update database to 'stopping' status (NOT inactive yet)
        # This tells the management loop the stream is being stopped intentionally
        await self.db_manager.execute_query(
            """UPDATE video_stream 
        ...
        logger.warning(
            f"🛑 STOP REQUESTED: stream={stream_id}, requester={requester_user_id}, "
            f"is_owner={is_owner}, is_admin={is_workspace_admin}"
        )
        
        # The actual cleanup (setting is_streaming=FALSE) will happen in _stop_stream
        # after the processing task completes
        
        return {
            "stream_id": str(stream_id),
            "name": stream_info['name'],
            "workspace_id": str(stream_info['workspace_id']),
            "message": "Stream stop initiated (will complete when processing finishes)"
        }
    ...
    async def restart_stream_in_workspace(
        ...
        """
        Restart stream with atomic transaction to prevent race conditions.
        CRITICAL: Uses transaction to ensure is_streaming stays TRUE throughout.
        """
        # Validate access
        stream_info, membership_info = await self.validate_workspace_stream_access(
            user_id=requester_user_id,
        ...
        # CRITICAL FIX: Use transaction to atomically stop and restart
        # This ensures is_streaming NEVER goes to FALSE
        async with self.db_manager.transaction() as conn:
            # Step 1: Signal existing task to stop (if running)
            async with self._lock:
                if stream_id_str in self.active_streams:
                    stop_event = self.active_streams[stream_id_str].get('stop_event')
                    if stop_event:
            ...
            # Step 2: Update to 'restarting' status (keeps is_streaming=TRUE!)
            await self.db_manager.execute_query(
                """UPDATE video_stream 
            ...
            # Wait briefly for old task to clean up
            await asyncio.sleep(10.0)
            
            # Step 3: Immediately set to 'processing' (still is_streaming=TRUE)
            await self.db_manager.execute_query(
                """UPDATE video_stream 
                SET is_streaming = TRUE,
                    status = 'processing', 
        ...
        logger.info(f"✅ Stream {stream_id} restart initiated atomically")
        
        # Management loop will see is_streaming=TRUE and start the stream
        
        return {
            "stream_id": str(stream_id),
            "name": stream_info['name'],
            "workspace_id": str(workspace_id),
            "message": "Stream restart completed atomically (management loop will process)"
        }
