...
class SharedVideoStream:
    ...
    def __init__(self, source: str, max_subscribers: int = 10):
        ...
        self.rtsp_timeout = 10  # seconds
        self.rtsp_reconnect_delay = 5  # seconds
        
        logging.info(f"Created SharedVideoStream for source: {source} "
                    f"(file: {self.is_file_source}, rtsp: {self.is_rtsp_source}, exists: {self.file_exists})")
    ...
    def _stop_capture(self):
        """Stop the video capture thread with force kill for RTSP"""
        ...
        # Release capture immediately to unblock read()
        if self.cap:
            try:
                self.cap.release()
                logging.debug(f"Released VideoCapture for {self.source}")
            except Exception as e:
                logging.warning(f"Error releasing capture: {e}")
            self.cap = None
        
        # Wait for thread with appropriate timeout
        if self.capture_thread and self.capture_thread.is_alive():
            # Longer timeout for RTSP to allow graceful shutdown
            ...
            if self.capture_thread.is_alive():
                logging.warning(f"⚠️ Capture thread for {self.source} did not stop within {timeout}s. "
                              f"Thread will be abandoned (daemon=True will clean it up).")
    ...
    def _configure_rtsp_environment(self):
        """Configure environment variables for RTSP streaming"""
        if not self.is_rtsp_source:
            return
        
        # Set FFmpeg options for RTSP (same as working RTSP_Stream_Reader.py)
    ...
    def _handle_read_failure(self):
        """Enhanced read failure handling for both files and RTSP"""
        self.consecutive_failures += 1
        
        # Special handling for file sources (video looping)
        if self.is_file_source and self.cap and self.cap.isOpened():
            try:
        ...
        # RTSP-specific reconnection logic
        elif self.is_rtsp_source:
            self.reconnect_attempts += 1
            
            if self.reconnect_attempts >= self.max_reconnect_attempts:
                logging.error(f"Max RTSP reconnect attempts ({self.max_reconnect_attempts}) reached for {self.source}")
                self.is_running = False
                return
            
            # Use fixed delay for RTSP (like working RTSP_Stream_Reader.py)
            delay = self.rtsp_reconnect_delay
            logging.warning(f"RTSP connection failure for {self.source}, "
                          f"attempt {self.reconnect_attempts}/{self.max_reconnect_attempts}. "
                          f"Retrying in {delay}s")
            
            time.sleep(delay)
            
            # Force re-open the RTSP connection
            if self.cap:
                self.cap.release()
                self.cap = None
        ...
        # Exponential backoff for other sources
    ...
    def _capture_loop(self):
        """Enhanced capture loop with RTSP support and timeout handling"""
        logging.info(f"Capture loop started for {self.source} (RTSP: {self.is_rtsp_source})")
        ...
        # Frame read timeout tracking
        last_successful_read = time.time()
        read_timeout = 30.0  # 30 seconds without frames = give up
        ...
        try:
            while not self.stop_capture.is_set() and self.is_running:
                try:
                    ...
                    # Check for stop signal before blocking read
                    if self.stop_capture.is_set() or not self.is_running:
                        break
                    
                    # Read frame (this can block)
                    ret, frame = self.cap.read()
                    
                    # Immediately check stop signal after read
                    if self.stop_capture.is_set() or not self.is_running:
                        break
                    ...
                    if not ret or frame is None:
                        ...
                        if self.is_rtsp_source:
                            ...
                            if time_since_success > read_timeout:
                                logging.error(f"No frames from {self.source} for {time_since_success:.1f}s, stopping")
                                break
                    ...
                    if self.is_rtsp_source:
                        # For RTSP, minimal delay to get real-time stream
                        sleep_time = 0.01
                    ...
                    # Sleep with stop check for RTSP (more responsive)
                    if self.is_rtsp_source and sleep_time > 0.01:
            ...
            if self.cap:
                try:
                    ...
                    logging.debug(f"Error releasing capture in finally: {e}")
                self.cap = None
    ...
    def _open_video_source(self) -> bool:
        """Enhanced video source opening with RTSP support"""
        try:
            if self.cap:
                self.cap.release()
                time.sleep(0.2)
            ...
            # RTSP-specific opening (like working RTSP_Stream_Reader.py)
            if self.is_rtsp_source:
                logging.info(f"Opening RTSP stream: {self.source}")
                
                # Ensure RTSP environment is configured
                self._configure_rtsp_environment()
                
                # Open with CAP_FFMPEG (required for RTSP)
                ...
                # Set RTSP-specific properties (like working code)
                ...
                if not ret or test_frame is None or test_frame.size == 0:
                    logging.error(f"RTSP stream opened but cannot read frames: {self.source}")
                    self.cap.release()
                    self.cap = None
                    return False
                ...
                logging.info(f"✓ RTSP stream connected: {self.source} - {width}x{height} @ {fps}fps")
                return True
            
            # File source opening
            elif self.is_file_source:
