...
class StreamManager:
    ...
    async def _periodic_cleanup(self):
        while True:
            try:
                ...
                # Delegate to qdrant_service for fire state cleanup
                await self.qdrant_service.cleanup_old_fire_states()
    ...
    async def start_stream_background(self, stream_id: UUID, owner_id: UUID, owner_username: str, 
        ...
        try:
            ...
            # Delegate Qdrant collection creation to qdrant_service
    ...
    async def start_stream_background_with_sharing(self, stream_id: UUID, owner_id: UUID, owner_username: str, 
        ...
        try:
            ...
            # Delegate fire state loading to qdrant_service
            await self.qdrant_service.load_fire_state_on_stream_start(stream_id)
            ...
            # Delegate to qdrant_service for collection creation
            await self.qdrant_service.ensure_workspace_collection(workspace_id)
    ...
    def detect_objects_with_threshold(self, frame: np.ndarray, conf_threshold: float = 0.4, 
        ...
        try:
            ...
            # Fire detection with proper state management
            if frame_count % 10 == 0:
                try:
    ...
    async def send_fire_alert_as_array(self, user_id: str, workspace_id: str, stream_id: str, camera_name: str, message: str):
        ...
        for ws in subscribers_for_user_copy:
            if ws.client_state == WebSocketState.CONNECTED:
                try:
                    await ws.send_json(fire_alert_array)
                    success_count += 1
        ...
        try:
            ...
            while not stop_event.is_set():
                try:
                    ...
                    # DELEGATE TO QDRANT_SERVICE
                    current_time = datetime.now(timezone.utc)

                    if fire_status in ["fire", "smoke"]:
                        # Get persistent state from qdrant_service
                        persistent_state = await self.qdrant_service.get_persistent_fire_state(stream_id)
                        ...
                        if should_send_notification:
                            try:
                                ...
                                # CRITICAL: Update database state with notification time via qdrant_service
                                await self.qdrant_service.update_persistent_fire_state(
                            ...
                            # Update detection state in database without notification via qdrant_service
                            await self.qdrant_service.update_persistent_fire_state(stream_id, fire_status, current_time)
                        ...
                        # Fire cleared - update database state via qdrant_service
                        persistent_state = await self.qdrant_service.get_persistent_fire_state(stream_id)
                        ...
                        await self.qdrant_service.update_persistent_fire_state(stream_id, "no detection")

                    # Always update in-memory fire state (for immediate reference)
                    self.fire_detection_states[stream_id_str] = fire_status

                    # Insert detection data if people detected - DELEGATE TO QDRANT_SERVICE
                    if person_count > 0:
